...
public class PresetEquipPart {
    ...
    public boolean isCurse(ServerPlayer serverPlayer){
        return ModGameRule.shouldCurseCheck(serverPlayer) && EnchantmentHelper.has(equipmentSlot.getItem(serverPlayer), EnchantmentEffectComponents.PREVENT_EQUIPMENT_DROP);
    }

    public boolean isCurse(ServerPlayer serverPlayer, MutableComponent component){
        boolean curse = this.isCurse(serverPlayer);
        if(curse && !component.contains(Utils.CursePart)) component.append(Utils.CursePart);
        return curse;
    }
    ...
    public boolean canFindPresetItem(ServerPlayer serverPlayer, MutableComponent component){
       ...
       if(!hasItem && !component.contains(Utils.LackPart)) component.append(Utils.LackPart);
       return hasItem;
    }
    ...
    public static PresetEquipPart fromTag(Player player, CompoundTag tag){
        SlotGetter slotGetter = SlotGetter.fromTag(tag.getCompound(EQUIP));
        ItemStack itemStack = ItemStack.parseOptional(player.registryAccess(),tag.getCompound(ITEM));
