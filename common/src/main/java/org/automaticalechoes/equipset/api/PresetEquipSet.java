...
public class PresetEquipSet extends LinkedHashMap<String, PresetEquipPart>{
    ...
    public PresetEquipSet(String name, Player player) {
        this.name = name;
        this.owner = player;
        initDefaultMap();
    }

    public PresetEquipSet(Player player, CompoundTag tag){
    ...
    private CompoundTag partsToTag(){
        ...
        for (Map.Entry<String, PresetEquipPart> entry : entrySet()) {
            tag.put(entry.getKey(), entry.getValue().toTag(owner));
        }
        return tag;
    }
    ...
    private void partsFromTag(CompoundTag tag){
        for (Map.Entry<String, SlotGetter> entry : DEFAULT_PARTS.entrySet()) {
            ...
            put(partName, tag.contains(partName) ? PresetEquipPart.fromTag (owner,tag.getCompound(partName)): PresetEquipPart.defaultSetting(entry.getValue()));
        }
    }
